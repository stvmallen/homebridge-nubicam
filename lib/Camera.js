...
let camApi = require('node-hikvision-api');
let URL = require('url');

class Camera {
    constructor(nubicam, log, Accessory, hap) {
        ...
        nubicam.getCameraFeed().then(feed => {
            ...
            camera.cameraAccessory.configureCameraSource(new FFMPEG(hap, camera.name, feed, log));
            ...
            camera.log.error("Failed initializing camera: %s", reason);
            camera.isInitialized = true;
        });
    }
    ...
    reportMotion(motionDetected) {
        this.log(motionDetected ? "Motion detected" : "Motion stopped");
