...
const FFMPEG = function (hap, cameraName, feed, log, enableAudio) {
    this.log = log;
    ...
    let isRtsp = false;//feed.rtsp !== undefined;
    let feedData = isRtsp ? feed.rtsp : feed;

    if (!feedData) {
        log.error("No source available for", cameraName);
        throw new Error("Missing source for camera: " + this.cameraName);
    }

    if (isRtsp) {
        let url = URL.parse(feedData);
        url.search = "";

        let rtspFeed = URL.format(url);

        this.videoSource = "-rtsp_transport tcp -vcodec h264_mmal -re -fflags +igndts -i " + rtspFeed;
        this.snapshotSource = "-i " + rtspFeed + "/picture";
    } else {
        this.feedSupplier = feedData;
    }
    ...
    log.debug("FFMPEG initialized for", cameraName);
};
...
FFMPEG.prototype = {
    ...
    handleSnapshotRequest(request, callback) {
        ...
        let imageSourcePromise = this.snapshotSource !== undefined ? new Promise(() => this.snapshotSource) : this.getVideoSource();

        imageSourcePromise.then(imageSource => {
            let ffmpegPath = pathToFfmpeg ? pathToFfmpeg : 'ffmpeg';
            let ffmpeg = spawn(ffmpegPath, (imageSource + ' -t 1 -s ' + resolution + ' -f image2 -').split(' '), {env: process.env});
    ...
    async getVideoSource() {
        return this.videoSource !== undefined ? this.videoSource : "-vcodec h264_mmal -fflags +igndts -i " + (await this.feedSupplier()).rtmp;
    },
                        ...
                        .then(videoSource => {
                            ...
                            if (this.enableAudio) {
                                ffmpegCommand += ' -map 0:a?' +
                                    ...
                                    ' srtp://' + targetAddress + ':' + targetAudioPort + '?rtcpport=' + targetAudioPort + '&localrtcpport=' + targetAudioPort + '&pkt_size=' + packetSize;
                            }

                            this.log.debug(ffmpegCommand);

                            let ffmpegPath = pathToFfmpeg ? pathToFfmpeg : 'ffmpeg';
                            let ffmpegSession = spawn(ffmpegPath, ffmpegCommand.split(' '), {env: process.env});
                            let that = this;
    ...
    createCameraControlService() {
        ...
        if (this.enableAudio) {
            this.services.push(new Service.Microphone());
        }
    },
